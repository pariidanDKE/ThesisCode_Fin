        ...
        for k in range(K):
            for i in range(n):
                ...
                for j in range(n):
                    if self.pred_comp[i][j] > 0:
                        ...
                        #f += np.asscalar(np.dot(ti.T, tj)) -  assscalar deprecated
                        f += np.ndarray.item(np.dot(ti.T, tj))
            ...
            if k < K - 1:  # Do not deflate for last component
                for i in range(n):
                    ...
                    #titi = np.asscalar(np.dot(ti.T, ti)) -  assscalar deprecated
                    titi = np.ndarray.item(np.dot(ti.T, ti))
...
#                 portho = portho / normpo
        ...
        #print("OnPLS model created with %d components! ftot = %f"
        #            % (numGlobalPredComp, ftot))
        ...
        for i in range(n):
            ...
            for j in range(n):
                if self.pred_comp[i][j] > 0:
                    ...
                    #
                    XjtXi = np.dot(X[j].T, X[i]) # DP: Get dot for pairwise blocks, fit pca
                    pca.fit(XjtXi)
                    if W is None:
                        W = pca.P
                    else:
